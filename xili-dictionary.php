...
define('XILIDICTIONARY_VER','0.9.6');
...
class xili_dictionary {
	...
	function xili_dictionary($langsfolder = '/') {
		...
		register_activation_hook(__FILE__,array(&$this,'xili_dictionary_activate'));
		...
		if(empty($xili_settings)) {
			define('DTAXONAME','dictionary');
			define('DTERMPREFIX','term_prefix');			
		} else {
			define('DTAXONAME',$xili_settings['taxonomy']);
			define('DTERMPREFIX',$xili_settings['term_prefix']);
		}
	...
	function xili_dictionary_activate() {
	...
	function xili_dictionary_settings() { 
		...
		if (isset($_GET['tagsnamesearch']))
			$tagsnamelike = $_GET['tagsnamesearch'];
	...
	switch($action) {
		...
		case 'add':
			...
			if (''!=$_POST['dictioline_lang']) {
				...
				$sslug = $_POST['dictioline_name'];
			}	
			$aliasof = $_POST['alias_of'];
			/*if ('' != $aliasof) {
				$posunder = strpos($aliasof,DTERMPREFIX);
				if ($posunder === false) {
					$aliasof = DTERMPREFIX.$aliasof;
				}	
			}*/
		    ...
		    if (is_wp_error($rterm_id)) $message .= ' ---- error ---- ';
		    $actiontype = "add";
		...
		case 'edited';
			...
			wp_update_term( $term, DTAXONAME, $args);
		...
		case 'deleting';
		    $actiontype = "add";
		    $term = $_POST['dictioline_term_id'];
		    wp_delete_term( $term, DTAXONAME, $args);
		...
		// select all terms if current term is new
			if ($term_id == 0) {
				...
				foreach ($listterms as $curterm) {
					if ($curterm->parent == 0)
						echo '<option value="'.$curterm->term_id.'" >'.__($curterm->name,'xili-dictionary').'</option>';
				} ?>
				</select>
				<br />
	...
	*/
	...
	function xili_import_in_tables($twintexts=Array(),$translang) {
	 ...
	 foreach ($twintexts as $key => $line) {
		...
		$cur_id = is_term($key, DTAXONAME);
		if ($cur_id == 0) {
			// create term
			//echo "no_exist ";
			$args = array('description' => $key);
			$result = wp_insert_term( $key, DTAXONAME, $args);
			echo $result['slug'];
			// keep is slug and id
			$parent_term = get_term($result['term_id'],DTAXONAME,OBJECT,'edit');
			// create slug and parent
			// verify translated term 
			$nbline++;
			if (is_term($line, DTAXONAME) == 0) {
				//echo "trans_no_exist ";
				$sslug = $parent_term->slug.'_'.strtolower($translang);
				$args = array('parent' => $parent_term->term_id, 'slug' => $sslug,'description' => $line);
				$result = wp_insert_term( $line, DTAXONAME, $args);
				//print_r($result); 
				$nbtline++;
			}	
		} else {
			...
			if (''!=$line) {
				if (is_term($line, DTAXONAME) == 0) {
					...
					$sslug = $parent_term->slug.'_'.strtolower($translang);
					$args = array('parent' => $parent_term->term_id, 'slug' => $sslug, 'description' => $line);
					if ($sslug != wp_unique_term_slug($sslug, $line)) { /* a new slug was given because sslug exists */
				...
				//echo '<br />------';
					...
					$sslug = $parent_term->slug.'_'.strtolower($translang);
					$args = array('parent' => $parent_term->term_id, 'slug' => $sslug, 'description' => $line);
					$existing = get_term_by('name', sanitize_title($line), DTAXONAME, OBJECT);
					$theterm = get_term_by('id',$existing->term_id, DTAXONAME, OBJECT);
						//echo $theterm->name;
					$exslug = $theterm->slug;
					if (substr_count($exslug,$parent_term->slug)>0 ) {
						/*possible because full term is in description */
						if ($sslug != wp_unique_term_slug($sslug, $theterm->name.'_'.strtolower($translang))) { /* a new slug was given because sslug exists */
							$existing = get_term_by('slug', $sslug, DTAXONAME, OBJECT);
							wp_update_term($existing->term_id, DTAXONAME, $args);
						} else {
							$result = wp_insert_term($theterm->name.'_'.strtolower($translang), DTAXONAME, $args);
						}
	 	...
	 	return array($nbline,$nbtline);  //root lines et translated lines;
	}
	...
	function xili_importcatsterms_in_tables($catterms= Array()){
	    ...
	    // verify if origin term exist
			...
			$cur_id = is_term($onecat[1], DTAXONAME);
			if ($cur_id == 0 && ""!=$onecat[1]) {
				// create term
				//echo "no_exist ";
				//$text = __('Category','xili-dictionary').' : '.$onecat[0];
				$args = array('description' => $onecat[1]);
				$result = wp_insert_term( $onecat[1], DTAXONAME, $args);
				$nbdesc++;
				//echo "------------- ".$onecat[1]."<----";
			}
		}
	...
	function xili_importthemeterms_in_tables($themeterms= Array()){
	    ...
	    // verify if origin term exist
			...
			if ($cur_id == 0) {
				...
				$result = wp_insert_term( $onecat, DTAXONAME, $args);
				$nbname++;
			}
		}
		return $nbname;
