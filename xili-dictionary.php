...
define('XILIDICTIONARY_VER','0.9.9');
...
class xili_dictionary {
	...
	/*list of dictiolines*/
		...
		if (empty($listdictiolines)) {
			...
			foreach ($listdictiolines as $dictioline) {
				...
				<td>".wp_filter_kses($dictioline->description)."</td>
	...
	*/
	...
	function xili_exportterms_inpo ($curlang) {
		...
		if (defined('THEME_LANGS_FOLDER')) {
			$langfolder = '/'.str_replace("/","",THEME_LANGS_FOLDER).'/' ;} else {$langfolder = "/"; 
		}
...
function dictionary_start () {
	$xili_dictionary = new xili_dictionary(); /* instantiation php4 for last century servers replace by =& */ 
}
add_action('plugins_loaded','dictionary_start'); 

/* © xiligroup dev 090617-13:00 */
