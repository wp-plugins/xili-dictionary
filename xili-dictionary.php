...
define('XILIDICTIONARY_VER','1.3.0');
...
class xili_dictionary {
    ...
    {
        /* Register our script. */
        wp_register_script('datatables', WP_PLUGIN_URL.'/'.dirname(plugin_basename(__FILE__)).'/js/jquery.dataTables.js' );
        wp_register_style('table_xdstyle', WP_PLUGIN_URL.'/'.dirname(plugin_basename(__FILE__)).'/css/xd_table.css');
    }
	...
	function on_normal_2_content($data) { 
			...
			<?php if ($action=='edit' || $action=='delete') { 
				...
				<th scope="row" valign="top" align="right" width="25%"><label for="dictioline_description1">
					...
					<?php if (isset($_GET['from_term_id'])) {
						...
						$parent_term = get_term($from_term_id,DTAXONAME,OBJECT,'edit');	
   	 ...
   	 ?>    
                	...
                	<?php if (isset($_GET['from_term_id'])) {
                	echo '<small> <-'.__("Don't forget to choose the language of translation !","xili-language").'</small>';
                	} ?>
	  			</td>
			</tr>
			<tr>
	    		...
	    		<?php 
	     		...
	     		if ($editterm->parent == 0) {
					...
					if (!empty($listterms)) {
						...
						echo '<a href="?page=dictionary_page&amp;from_term_id='.$term_id.'">'.__('add a translation','xili-dictionary')."</a><br /><br />";		
					}	
	     		} else {
						...
						if (substr($curchildterm->slug,-5) == strtolower($curlang)) { 
							if ($obj == 'mo') {
								if (false === strpos($curterm->description,XPLURAL)) {
									$mo->add_entry($mo->make_entry($curterm->description, $curchildterm->description)); 								} else {
