...
define('XILIDICTIONARY_VER','2.1-rc1');
	 ...
	 */
	 function print_styles_xdmsg_edit ( ) { 
	 	...
	 	if ( get_post_type( $post->ID ) == XDMSG ) {
			...
			echo '#msg-states-actions .xdversion { font-size:80%; text-align:right; }'."\n";
...
<div id="msg-states-actions" >
	<strong><?php _e( 'Informations and actions about files .po / mo', 'xili-language' ); echo ':</strong><br />'; ?>
...
<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;">
	...
	<p> <?php _e( 'External file xd-style.css for dashboard (flags, customization)','xili-dictionary' ); ?></p>
		...
		?>
   		<a style="<?php echo $linkstyle2 ; ?>" href="<?php echo $nonce_url ?>" title="<?php _e( 'Change style mode', 'xili-dictionary') ?>"><?php _e( $style_action ) ?></a>
...
<a style="<?php echo $linkstyle2 ; ?>" href="<?php echo $this->xd_settings_page.'&amp;action=importbloginfos'; ?>" title="<?php _e('Import infos of web site and more','xili-dictionary') ?>"><?php _e("Import terms of website's infos",'xili-dictionary') ?></a>
	...
	<h4 id="manage_dictionary"><?php _e('Dictionary','xili-dictionary') ;?></h4>
   		<a style="<?php echo $linkstyle2 ; ?>" href="?post_type=xdmsg&action=erasedictionary&amp;page=dictionary_page" title="<?php _e('Erase all terms of dictionary ! (but not .mo or .po files)','xili-dictionary') ?>"><?php _e('Erase all terms','xili-dictionary') ?></a>
		...
		case 'importingmo';  // MO
		    ...
		    if ( isset ( $_POST['local-import'] ) && $_POST['local-import'] == 'local-import' ) {
		    	if ( is_multisite() ) {
		    		...
		    		$po = $this->pomo_import_PO ( $selectlang, $folder_file, true ); // - only in site if model
		    	
		    	} else {
		    		
		    		$po = $this->pomo_import_PO ( $selectlang, '', true ); // file set in function
		    	}
		    } else {
...
<div id="xili-dictionary-settings" class="wrap columns-2" style="min-width:850px">
	...
	function pomo_import_PO ( $lang = "", $local = false ) {
		...
		if ( $local == false ) {
			...
			$pofile = $this->get_template_directory.$this->langfolder.'lang-'.$lang.'.po';
		}
	 ...
	 * @since 2.0
	 */
	function get_cpt_msgids( $curlang, $pomo = "mo", $extract_array = array() ) {
		global $wpdb;
