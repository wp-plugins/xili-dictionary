...
define('XILIDICTIONARY_VER','2.7.0');
	...
	<label for="only-theme">
		...
		<input id="only-theme" name="only-theme" type="checkbox" value="only" />
	     	...
	     	if ( $action=='export' || $action=='exportpo' ) { ?>
				...
				<br /><br />&nbsp;&nbsp;<label for="languagesfolder">
			     	...
			     	if ( is_multisite() ) { /* complete theme's language with db structure languages (cats, desc,…) in uploads */
				     	...
				     	if ( isset ( $_POST['only-local'] ) && $_POST['only-local'] == 'local' ) {
				     		...
				     		if ( $_POST['languagesfolder'] == 'parenttheme' ) {
				     			...
				     			if ( $_POST['languagesfolder'] == 'contentlanguages' ) {
				     				$file = WP_LANG_DIR . '/themes/' . $this->theme_domain() .'-' . $selectlang . '.po' ;
				     			} else {
	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xd_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'This version %s adds ways to built local- language files in wp-content/languages subfolder.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';

				$pointer_text .= '<p>' . esc_js( sprintf(__( 'Previous version to %s adds ways to translate language files of plugins.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';
