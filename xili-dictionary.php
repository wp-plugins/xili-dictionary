...
define( 'XILIDICTIONARY_VER', '2.9.0' );
...
<div class="metabox-content" >
	...
	<?php // middle column
		...
		if ( $do_it ) {
		?>
			<br /><br /><label for="target"><input class="target" name="target" type="radio" value="targetplugin" />&nbsp;<?php _e('Plugin (select one origin at right):','xili-dictionary'); ?>
		</label>
		<?php }
		}
		?>
...
<script type="text/javascript">
	...
	function update_ui_state () {
		...
		if ( jQuery('#only-local').attr('checked') ) place = 'local';
		var la = jQuery('#language_file').val();
		var target = jQuery('.target:checked').val();
		if ( target == "targetplugin" ) {
			place = 'plugin';
		...
		var a = from_file_exists( 'files', place, curthemename, plugin, la, x ); //alert ( a + ' <> ' + place + curthemename + plugin + la + x );
		show_file_states( 'files', place, curthemename, plugin, la, x );
		if ( a == "exists" ) {
	...
	jQuery('#language_file , #languagesfolder, #only-local, .target, table.checktheme input:checkbox').change(function() {
		update_ui_state ();
	});
...
<div id="xili-dictionary-settings" class="wrap columns-2" style="min-width:850px">
	 ...
	 *
	 */
	function from_entries_to_xdmsg ( $originals, $origin_theme = 'twentyfourteen-xili', $backup_pot = array() ) {
		...
		foreach ( $temp_po->entries as $pomsgid => $pomsgstr ) {
			...
			$this->pomo_entry_to_xdmsg ( $pomsgid, $pomsgstr, '', array( 'importing_po_comments'=>'replace', 'origin_theme'=>$origin_theme ) );
			$lines++;
		}
		$this->importing_mode = false;
		if ( $backup_pot ) {
	...
	function display_file_states ( $file_state ) {
		...
		if ( $file_state['file_exists'] ) {
			...
			$return .= sprintf(" <strong>%-40s</strong> [%swritable]<br />", basename( $file_state['file'] ), $file_state['is_writable'] ? "" : "not ");
		}
	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xd_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'This version %s adds ways to scan sources files (current theme or available plugins) and import translatable terms.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';

				$pointer_text .= '<p>' . esc_js( sprintf(__( 'Previous version of %s adds ways to built .pot files.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';
	...
	*/
	...
	function build_file_full_path ( $request ) {
		...
		if ( $request['fromwhat'] == 'sources' ) {
			if ( $request['place'] == 'theme' ) {
				$origin_theme = $this->get_option_theme_name( false ) ;


				$file = $this->active_theme_directory . $this->langfolder . $theme_text_domain .'.pot' ;
			...
			$base = $base . $lang;
			$file = str_replace ( '//', '/' ,  $path . $base . '.' . $suffix ); //error_log ($file);
		}
		return $file ;
	}
	...
	function admin_head() {
		...
		?>
			<script type="text/javascript">
			//<![CDATA[
			function show_file_states( fromwhat, place, curthemename, plugin, la, x ) {
			...
			function from_file_exists ( fromwhat, place, curthemename, plugin, la, x ) {
	...
	*/
	...
	jQuery(document).ready( function() {
		...
		if ( fromwhat == 'sources' ) {
			...
			var sources = { 'theme' : '<?php echo esc_js(__( 'this theme', 'xili-dictionary' )); ?>', 'plugin' : '<?php echo esc_js(__( 'this plugin', 'xili-dictionary' )); ?>'}
		} else {
		...
		if ( place == 'local' ) {
			...
			if ( fromwhat == 'sources' ) {
				if ( 'string' == typeof (plugin) && plugin != '' ) submitval = t+' '+sources[place]+' : '+ plugindatas[plugin]['domain'] ;
				if ( 'string' == typeof (plugin) && plugin != '' ) jQuery('#backup_pot_label').html( '('+ plugindatas[plugin]['domain'] + '.pot)' );
			} else {
				if ( la == pluginpotfile ) {
			...
			if ( 'string' == typeof (plugin) && plugin != '' ) jQuery(".title_origin_theme").html( plugindatas[plugin]['domain'] );
			//
		} else {
			if ( fromwhat == 'sources' ) {
				submitval = t+' : ? ' ;
		  	...
		  	show_file_states( fromwhat, place, curthemename, plugin, la, x );
		  	jQuery("#xd-looping-start").val( submitval );
		  	if ( fromwhat == 'files' ) {
		  		var a = from_file_exists( fromwhat, place, curthemename, plugin, la, x );
		  		if ( a == 'exists' ) {
		  	...
		  	if ( fromwhat == 'sources' ) {
		  		if ( jQuery('#backup_pot').attr('checked') ) {
		  			jQuery( '#xd-file-state' ).show();
		  		} else {
					jQuery( '#xd-file-state' ).hide() ;
				}
		...
		jQuery('#backup_pot').change(function() {
			bbtvalue ( '' );
		});
...
?>
	...
	function xd_file_scanning_setting_callback_row() {
		...
		echo '<br />' . __('Backup in pot file', 'xili-dictionary' ). ' <span id="backup_pot_label"></span> <input id="backup_pot" name="backup_pot" type="checkbox" />' ;
	}
	...
	function importing_process_callback () {
		...
		if ( $from_what == 'sources') {
			$lang = '';
			$create_pot_file = ( isset($_POST['backup_pot']) ) ? true : false ;
		}
	...
	private static function caching_pot_obj ( $place, $plugin_key, $backup_pot = false ) {
		...
		if ( $place == 'plugin' && $plugin_key != '' ) {
			...
			if ( isset ($list_plugins[$plugin_key]) && $project_key != '.' ) {
				...
				$params = ( isset( $_POST['backup_pot'] ) ) ? $projects[$project_key] : array() ;
			...
			$the_theme_project = array(
				'title'    => $origin_theme . ' generated by xili-dictionary',
				'file'     => $xili_dictionary->active_theme_directory . $xili_dictionary->langfolder . $theme_text_domain .'.pot',
				'excludes' => array(),
				'includes' => array(),
				'working_path' => get_stylesheet_directory(),
				) ;
