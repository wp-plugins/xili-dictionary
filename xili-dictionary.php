...
define( 'XILIDICTIONARY_VER', '2.11.1' );
...
*/ ?>
	...
	function xili_dictionary_settings() {
		...
		case 'importbloginfos'; // bloginfos and others since 1.1.0
			...
			. '<br />' . __('The process will import around 130 <strong>msgid</strong> from sources, so be patient.','xili-dictionary');
			$UI_lang = get_locale();
...
<div id="xili-dictionary-settings" class="wrap columns-2" style="min-width:850px">
	...
	function xili_import_infosterms_cpt () {
		...
		if ( class_exists ('xili_language') ) {
			...
			$nbname[0] += $this->import_msgid_from_one_wp_file ( ABSPATH . WPINC, "comment-template", $xili_language->comment_form_labels, true, true ); // local + msgstr
	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xd_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'In this version %s, if you switch language of dashboard is other than in en_US, then the import process of sources msgid will try to import translations of chosen language.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'Previous version of %s adds way to built only .pot files w/o using dictionary. (also tested with new WP 4.2)', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';
