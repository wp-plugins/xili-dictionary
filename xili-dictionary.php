...
define('XILIDICTIONARY_VER','1.0.5');
...
class xili_dictionary {
		     	...
		     	if ($this->notwp3 === false) {
			     	if (is_multisite()) { /* complete theme's language with db structure languages (cats, desc,…) in uploads */
	    				...
	    				//$thesite_ID = $wpdb->blogid;
	   					...
	   					$mo = $this->from_twin_to_POMO_wpmu ($selectlang, $superadmin);
	...
	function from_twin_to_POMO_wpmu ($curlang, $obj='mo', $superadmin = false)	{
	    ...
	    // the pomo array available in theme's folder 
	   	...
	   	// array diff
	   	$site_mo = $table_mo;
	   	if (false !== $theme_mo && false  === $superadmin) // special for superadmin who don't need diff.
	   		$site_mo->entries =  array_diff_key($table_mo->entries,$theme_mo->entries); // without keys available in theme' mo
