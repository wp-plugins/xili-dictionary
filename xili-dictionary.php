...
define('XILIDICTIONARY_VER','2.0.0-rc1');
...
<div id="msg-states">
	  <?php
	  // $this->msg_link_display ( $post_ID , true );
...
<div id="xili-dictionary-settings" class="wrap" style="min-width:850px">
	...
	<div id="message" class="updated fade"><p><?php echo $themessages[$msg]; ?></p></div>
			...
			/****** tests ****/
			//echo $this->get_cpt_msgids ( 'fr_FR' );
			
			//if ( $result = $this->msgid_exists ( '<span class="%1$s">Posted in</span> %2$s' )  ) { print_r( $result ); } else { echo 'nothing'; } ; 
	...
	function get_cpt_msgids( $curlang, $pomo = "mo" ) {
		global $wpdb;
		//$lang = term_exists( $curlang , TAXONAME) ;
		if ( $pomo == "mo" ) {
	...
	/** 
	 * cat's terms in array (name - description)*/
