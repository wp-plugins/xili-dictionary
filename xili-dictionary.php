...
define( 'XILIDICTIONARY_VER', '2.10.0' );
		...
		if ( $action=='export' || $action=='exportpo' ) { ?>
			...
			&nbsp;<select id="only-local" name="only-local">
				<?php
					...
					echo '<option value="local" >' . sprintf( __('Save only the local-xx_XX.%s sub-selection','xili-dictionary'), $filetype);
...
<div id="xili-dictionary-settings" class="wrap columns-2" style="min-width:850px">
	...
	function xili_import_infosterms_cpt () {
		...
		if ( class_exists ('xili_language') ) {
			...
			// added 2.10.0 to extract comment default msgid
			$nbname[0] += $this->import_msgid_from_one_wp_file ( ABSPATH . WPINC, "comment-template", $xili_language->comment_form_labels, true ); // local
...
?>
	 ...
	 *
 	 * @since 2.10.0
 	 */
	function import_msgid_from_one_wp_file ( $path, $file, $entries_to_exclude, $local = true ) {
		global $wp_version;
		$entries = false;
		...
		$lines = $this->from_entries_to_xdmsg( $entries, $file, false );
