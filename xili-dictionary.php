...
define('XILIDICTIONARY_VER','2.5.0');
	...
	function xili_manage_column_row  ( $column , $id ) {
		...
		if ($column == 'msgposttype' && $post->post_type == XDMSG ) {
			...
			if ( $origins ) {
				...
				printf( __('<small>From theme(s):</small> %s', 'xili-dictionary' ) , implode (', ', $names ) ) ;
			}
		}
...
<div style="background:#f5f5fe;">
	<div class="dialogcontainer" >
		...
		if ( in_array ( $action , array ( 'import', 'importmo', 'importbloginfos', 'importtaxonomy', 'importcurthemeterms', 'importpluginmsgs' ) ) ) {
		...
		if ( in_array( $action, array ( 'export', 'importmo', 'import', 'exportpo', 'download' ) ) ) {
...
<div id="xili-dictionary-settings" class="wrap columns-2" style="min-width:850px">
	 ...
	 */
	 function add_help_text($contextual_help, $screen_id, $screen) {
	  ...
	  if ( $screen->id == 'edit-category' ) { // 2.3.8
...
<?php if ($msg != '') : ?>
					...
					<?php
						$class_error = ( $msg != 'ok' ) ? ' error-message' : '' ;
...
<?php endif; ?>
	...
	function _xd_download_type_define( $input ) { //error_log('**** define ****');
	...
	function deliver_file ($selectlang, $type_file, $sub_type_file, $place_file, $compress_file = '' ){
		...
		$diskfile = $path . $filename; //error_log ( $diskfile .' d-f '. $filename );
...
/** inspirated form wp-db-backup */
	function transfer_file ( $diskfile = '', $filename = '', $compress_file = '' ) {
		...
		if ( file_exists( $diskfile && $compress_file == 'gz') ) {
			...
			//error_log('yes zip >' . $zip_diskfile ) ;
		}
