...
define('XILIDICTIONARY_VER','2.8.0');
...
<div class="metabox-content" >
	...
	<?php // middle column
		...
		if ( $action=='export' || $action=='exportpo' ) { ?>
			...
			<label for="target">
				<input checked="checked" name="target" type="radio" value="targettheme" />&nbsp;<?php _e('Theme','xili-dictionary'); ?>
			</label>
		...
		if ($do_it) {
		?>
			<br /><br /><label for="target"><input name="target" type="radio" value="targetplugin" />&nbsp;<?php _e('Plugin (select one origin at right):','xili-dictionary'); ?>
		</label>
		<?php }
		}
		?>
			...
			if ("" != $selectlang) {
				...
				if ( $_POST['target'] == "targettheme" ) {
					...
					if ( $checked_themes != array() ) {
						...
						$file = $this->path_plugin_file( $checked_themes[0], $selectlang, substr ( $file_suffix, 1 ) );
					}
				}

				if ( count ($po->entries) > 0 ){ // 2.2
					$local = ( $_POST['target'] == "targetplugin" ) ? '&nbsp;-&nbsp;'.basename( $file, $file_suffix ) : $local;
	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xd_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'This version %s adds ways to built .pot files.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';

				$pointer_text .= '<p>' . esc_js( sprintf(__( 'Previous version of %s adds ways to built local- language files in wp-content/languages subfolder.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';
