...
define('XILIDICTIONARY_VER','1.4.4');
...
class xili_dictionary {
	...
	switch($action) {
			...
			if ( $possible ) {
			    ...
			    if (''!=$_POST['dictioline_lang']) {
				    if (is_wp_error($rterm_id)) { 
				    	$message .= ' ---- error --lang-- ' . $rterm_id->get_error_code() ;
				    	$msg = 8;
				    } else {
			    	...
			    	if (is_wp_error($rterm_id)) { 
			    		$message .= ' ---- error ---- ' . $rterm_id->get_error_code()  ;
			    		$msg = 8; 
			    	} else { 
			    		$message .= " (". $rterm_id['term_id'] .") ";
			    		$msg = 1;
			    	}
			    }
			}
		...
		case 'edited';
			...
			if ( $parent_id != 0 ) {
			    if ( is_wp_error($rterm_id) ) { 
			    	$message .= ' ---- child error ---- '. $rterm_id->get_error_code();
			    	$msg = 9;
			    } else {
			    	$this->xd_wp_set_object_terms((int) $rterm_id['term_id'], $_POST['dictioline_lang'], XDDICTLANGS,false);
