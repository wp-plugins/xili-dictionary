...
define('XILIDICTIONARY_VER','2.1.3');
...
class xili_dictionary {
	...
	public function __construct( $langsfolder = '/' ) {	
		...
		if ( is_admin() ) {
			...
			add_action( 'add_meta_boxes_' . XDMSG, array(&$this, 'msg_update_action')); // do_action('add_meta_boxes_' . $post_type, $post);
		}
	...
	function add_custom_box_in_post_msg () {
		...
		if ( get_current_screen()->action != 'add' ) { // only for edit not new
			...
			add_meta_box('msg_tools_shortcuts', __("Shortcuts to create mo files",'xili-dictionary'), array(&$this,'msg_tools_shortcuts'), XDMSG , 'side','high');
		}
	}
	 ...
	 * 
	 */
	function msg_tools_shortcuts ( $post ) {
		$post_ID = $post->ID;
		$lang = $this->cur_lang ( $post_ID );
	  	...
	  	if ( $lang ) {
	  		...
	  		echo '<p>' . sprintf('<a class="action-button grey-button" onClick="verifybefore(0)" href="%1$s" >'.__('Update','xili-dictionary').' %3$s.mo</a><br /><a class="action-button blue-button" onClick="verifybefore(1)" href="%2$s" >'.__('Update','xili-dictionary').' local-%3$s.mo</a>','#', '#', $lang->name).'</p>';
...
function verifybefore(id) {
  ...
  if (confirm(confirmmessage)) {
	...
	function msg_post_messages ( $messages ) {
		$messages['post'][33] = __('MO file updated', 'xili-dictionary');
		$messages['post'][34] = __('Local MO file updated', 'xili-dictionary');
		return $messages;
	}
			...
			if ( is_multisite() ) {
	     	...
	     	if ( false === $this->Save_MO_to_file ( $selectlang , $mo, $file ) ) {
				$this->msg_action_message = sprintf('<span class="alert">'.__('Error with File %s !', 'xili-dictionary').'</span> ('.$file.')', $local.$selectlang.'.mo');
	     		...
	     		$this->msg_action_message = sprintf(__('File %s updated', 'xili-dictionary'), $local.$selectlang.'.mo');
	     	}
...
<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;">
	...
	function  on_sidebox_style_content() { 
		?>
			...
			$selectlang = $_POST['language_file'];
		     if ("" != $selectlang){
		     	...
		     	if ( false === $this->Save_MO_to_file ($selectlang , $mo, $file ) ) { 
		     		...
		     		$message .= ' '.sprintf(__('exported in %2s %1s.mo file.','xili-dictionary'),$selectlang, $local);
		     	}
		     }	else {
			...
			$selectlang = $_POST['language_file'];
		     if ("" != $selectlang){
		     	...
		     	if ( false === $this->Save_PO_to_file ( $selectlang , $po, $file ) ) {	
		     		...
		     		$message .= ' '.sprintf(__('exported in %2s %1s.po file.','xili-dictionary'), $selectlang, $local);
		     	}	
		     } else {
		     	$message .= ' : error "'.$selectlang.'"';
		     }	
		...
		case 'importingmo';  // MO
		    ...
		    if ( isset ( $_POST['local-import'] ) && $_POST['local-import'] == 'local-import' ) {
		    	...
		    	if ( is_multisite() ) {
		    		...
		    		$po = $this->pomo_import_MO ( $selectlang, $folder_file, true ); // - only in site if model
		    	
		    	} else {
		    		
		    		$po = $this->pomo_import_MO ( $selectlang, '', true ); // file set in function
		    	}
		    } else {
		    	$local = '';
	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xd_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( __( 'New boxes added in edit screen of a term: list of untranslated terms, shorcuts to update quickly .mo file', 'xili-dictionary')). ',</p>';
...
add_action( 'plugins_loaded', 'xili_dictionary_start', 20 ); // after xili-language and xili_tidy_tags

/* © xiligroup dev 20120819 */
