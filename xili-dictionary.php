...
define('XILIDICTIONARY_VER','2.3.7');
	...
	function xili_dictionary_settings() { 
		...
		case 'importing';  // PO
		    ...
		    if (false !== $po ) {
		    	...
		    	$readfile = $this->active_theme_directory.$this->langfolder.$local.$selectlang.'.po';
		...
		case 'importingmo';  // MO
		    ...
		    if (false !== $mo ) {
		    	...
		    	$readfile = $this->active_theme_directory.$this->langfolder.$local.$selectlang.'.mo';
			...
			if ( is_multisite() && $multilocal ) {
				...
				if ( !is_child_theme() ) {
					...
					if ( 'parenttheme' == $local ) {
						$path = get_template_directory() . $langfolder;
					} else {
						$path = get_stylesheet_directory() . $langfolder;
					}
				}
			}
