...
# 2.6.1 - 140310 - fixes issues with pot file name and verify no msgstr when importing pot file.
...
define('XILIDICTIONARY_VER','2.6.1');
		...
		$this->theme_mos = $this->get_pomo_from_theme();
 		...
 		if ( $list_plugins ) {
 			foreach ($list_plugins as $plugin_path ){
				$this->plugin_mos[$plugin_path] = $this->get_pomo_from_plugin($plugin_path);
			}
		}
	}
		...
		if ( isset ($_GET['msgupdate'] ) && isset ($_GET['langstr']) ) { // shortcut to update .mo - 2.1.2
			...
			if ( is_multisite() ) {
				if (($uploads = xili_upload_dir()) && false === $uploads['error'] ) {
     				...
     				$extract_array['projet_id_version'] = 'theme = ' . $theme_name ;
...
<div style="background:#f5f5fe;">
	<div class="dialogcontainer" >
		...
		if ( in_array ( $action , array ( 'importbloginfos', 'importtaxonomy', 'importcurthemeterms', 'importpluginmsgs' ) ) ) {
	...
	<label for="only-theme">
		...
		<input id="only-theme" name="only-theme" type="checkbox" value="only" />
	     	...
	     	if ( $action=='export' || $action=='exportpo' ) { ?>
	     		...
	     		<label for="target">
	     			<input checked="checked" name="target" type="radio" value="targettheme" />&nbsp;<?php _e('Theme languages subfolder','xili-dictionary'); ?>
				</label>
				...
				<br /><br />&nbsp;&nbsp;<label for="parenttheme">
		     		<?php _e('Save in parent theme languages subfolder','xili-dictionary'); ?>
					&nbsp;<input id="parenttheme" name="parenttheme" type="checkbox" value="parenttheme" />
				</label>

	     	<?php }
	     	if ($do_it) {
	     	?>
	     		<br /><br /><label for="target"><input name="target" type="radio" value="targetplugin" />&nbsp;<?php _e('Plugin languages subfolder (select one origin at right):','xili-dictionary'); ?>
				</label>
	     	<?php }
	     	}
	     	?>
			...
			echo '<div class="dialogorigin">';
				if ( $listterms ) {
	 			...
	 			foreach ( $listterms as $onetheme) {
	 				...
	 				$checkline .= '<td><input type="checkbox" '. $checked .' id="theme-'.$onetheme->term_id.'" name="theme-'.$onetheme->term_id.'" value="'.$onetheme->slug.'" />&nbsp;' . $title .'</td>';
	 				$i++;
	...
	<div class="dialogbbt">
		...
		if ( $action == 'importpluginmsgs' && $checked_domains != array() ) {
			...
			?>
			<br />
		<?php } else if ( in_array ( $action, array ( 'collectingpluginmsgs', 'importpluginmsgs' ) ) ) {
...
<fieldset style="margin:2px; padding:3px; border:1px solid #ccc;">
	...
	function xili_dictionary_settings() {
		...
		case 'export';
			 ...
			 $formhow = __('To build a .mo file, choose language and click below.','xili-dictionary');
			 $submit_text = __('Build &raquo;','xili-dictionary');
		     break;
			     	...
			     	if ( is_multisite() ) { /* complete theme's language with db structure languages (cats, desc,…) in uploads */
	    				...
	    				//$thesite_ID = $wpdb->blogid;
			     		...
			     		if ( isset ( $_POST['only-local'] ) && $_POST['only-local'] == 'local' ) {
			     			...
			     			$extract_array [ $this->msg_extracted_comments ] = $this->local_tag;
				     		$extract_array [ 'like-'.$this->msg_extracted_comments ] = true;
				     		if ( isset ( $_POST['parenttheme'] ) && $_POST['parenttheme'] == 'parenttheme' ) {
				     			$file = get_template_directory().$this->parentlangfolder.'local-'.$selectlang.'.mo' ;
				     		} else {
			     				$file = $this->active_theme_directory.$this->langfolder.'local-'.$selectlang.'.mo' ;
				     		}
		...
		case 'exportpo';
			 ...
			 $formhow = __('To export terms and build a .po file, choose language and click below.','xili-dictionary');
			 $submit_text = __('Build &raquo;','xili-dictionary');
		     break;
		     	...
		     	if ( $_POST['target'] == "targettheme" ) {
			     	if ( is_multisite() ) { /* complete theme's language with db structure languages (cats, desc,…) in uploads */
				     	...
				     	if ( isset ( $_POST['only-local'] ) && $_POST['only-local'] == 'local' ) {
				     		...
				     		if ( isset ( $_POST['parenttheme'] ) && $_POST['parenttheme'] == 'parenttheme' ) {
				     			$file = get_template_directory().$this->parentlangfolder.$selectlang.'.po' ;
				     		} else {
			     				$file = $this->active_theme_directory.$this->langfolder.$selectlang.'.po' ;
				     		}
				     	}
	    			}
	    			$extract_array['projet_id_version'] = 'theme = ' . $cur_theme_name ;
			     	...
			     	if ( $checked_themes != array() ) {
						...
						$extract_array['projet_id_version'] = 'plugin = ' . $checked_themes[0] ;
						$po = $this->from_cpt_to_POMO ( $selectlang, 'po', $extract_array );
						$file = $this->path_plugin_file( $checked_themes[0], $selectlang, 'po' );
					}
			     }
		     	if ( count ($po->entries) > 0 ){ // 2.2
		     		$local = ( $_POST['target'] == "targetplugin" ) ?  '&nbsp;-&nbsp;'.basename($file, '.po') : $local;
		...
		case 'importbloginfos'; // bloginfos and others since 1.1.0
		    ...
		    $formhow = __('To import terms of blog info and others defining this current website (title, date,...), click below.','xili-dictionary');
...
<div id="xili-dictionary-settings" class="wrap columns-2" style="min-width:850px">
	 ...
	 */
	 ...
	 function get_pomo_from_plugin( $plugin_path ) {
	 	if (isset ( $plugin_mos[$plugin_path] )) $plugin_mos[$plugin_path] = array();
	 	if ( defined ('TAXONAME') ) {
	 		$listlanguages = $this->get_terms_of_groups_lite ( $this->langs_group_id, TAXOLANGSGROUP, TAXONAME, 'ASC' ); //get_terms(TAXONAME, array('hide_empty' => false));

	 		foreach ($listlanguages as $reflanguage) {
	     		$res = $this->pomo_import_MO ( $reflanguage->name, '', false , $plugin_path);
	     		if ( false !== $res )
	     			$plugin_mos[$plugin_path][$reflanguage->slug] = $res->entries;
	 		}
	 	}
	 	return $plugin_mos[$plugin_path] ;
	 }
	 ...
	 */
	function pomo_import_MO ($lang = "", $mofile = "", $local = false, $plugin_path = false ) {
		$mo = new MO();

		if ( $plugin_path ) {
			$mofile = $this->path_plugin_file( $plugin_path, $lang, 'mo' );
		} else {
	...
	function Save_PO_to_file ($curlang , $po, $createfile = ""  )	{
		...
		if ( "" == $createfile)
			$createfile = $this->active_theme_directory . $this->langfolder . $filename;
			if ( defined ('WP_DEBUG') &&  WP_DEBUG != true ) error_log ( '---- XD po file ------- '.$createfile );
	...
	function xili_import_infosterms_cpt () {
		...
		if ( class_exists ('xili_language') ) {
			...
			$listlanguages = $this->get_terms_of_groups_lite ( $this->langs_group_id, TAXOLANGSGROUP,TAXONAME, 'ASC' ); //get_terms(TAXONAME, array('hide_empty' => false));
	 		...
	 		$nbname[0] += 3 * count( $listlanguages );
		...
		if ( class_exists ('xili_language') && version_compare ( XILILANGUAGE_VER, '2.3.9', '>' ) && 'en_US' != $curlang ) {
	...
	function scan_import_theme_terms( $callback, $display ) {
		$path = $this->active_theme_directory ;
		$themefiles = array();
		...
		if ( isset ( $collected_msgs[$locale] ) && is_array( $collected_msgs[$locale] ) && $collected_msgs[$locale] != array() ) {
			...
			foreach ( $collected_msgs[$locale] as $oneline ) {
				...
				if ( $result === false ) {
					...
					wp_set_object_terms( $msgstr_post_ID, $locale, TAXONAME );
				} else {
					$msgstr_post_ID = $result[0];
				}
	  ...
	  if ( 'xdmsg_page_dictionary_page' == $screen->id ) {
	  	...
	  	if ( 'edit-xdmsg' == $screen->id ) {
			...
			$about_infos_edit_msg =
		      ...
		      '<li>' . __('It is possible to filter according language of Origin of msg.','xili-dictionary') . '</li>' .
		      ...
		      '</ul>' .
		      '<p>' . __('More details in future releases.','xili-dictionary') . '</p>';
	  	...
	  	$screen->add_help_tab( array(
	 		...
	 		$to_remember =
		      ...
		      '<li>' . __('When the selection is successful, the link appears in the message space under the title.','xili-dictionary') . '</li>' .
		      '</ul>'.
		      '<p>' . sprintf(__('<a href="%s" target="_blank">Xili Wiki Documentation</a>','xili-dictionary'), $wikilink ) . '</p>'  ;
	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xd_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'This version %s adds ways to translate language files of plugins.', 'xili-dictionary'), XILIDICTIONARY_VER) ). ',</p>';

				$pointer_text .= '<p>' . esc_js( __( 'Previous version introduces way to download language files to your computer.', 'xili-dictionary')). ',</p>';
...
<?php if ($msg != '') : ?>
					...
					<?php
						...
						if ( 'error' == $msg ) {
							...
							$place = ($_POST['download_place_file'] == 'parent') ? __('Parent theme', 'xili-dictionary'): __('Child theme', 'xili-dictionary');
							echo '”&nbsp;' . __('from','xili-dictionary') . '&nbsp;' .$place ;
						}

						if  ($this->download_uri ) {
							$basename = basename ( $this->download_uri );
							echo ' <a href="'. $this->download_uri . '" /><strong>' . sprintf(__('Click here for download file %s', 'xili-dictionary'), $basename ) . '</strong></a>' ;
						}
					?></p></div>
<?php endif; ?>
			<form enctype="multipart/form-data" action="/wp_svn38/wp-admin/edit.php?post_type=xdmsg&amp;page=download_dictionary_page" method="post" id="xd-download-settings">
	...
	function xd_download_setting_callback_main_section() {
		echo '<p>'.__('Here it is possible to choose a language file to download to your computer for further works or archives.', 'xili-dictionary' ) . '</p>';
	}
	...
	function xd_file_download_setting_callback_row() {
		...
		<p class="description"><?php _e( "Language of the file or POT type if available in theme's folder when importing .po file.", 'xili-dictionary' ); ?></p>
</div><br />
		...
		<div class="sub-field">
			...
			<select name="download_sub_type_file" id="download_sub_type_file">
				...
				<p class="description"><?php _e( 'Subtype of file: .mo or .po', 'xili-dictionary' ); ?></p>
		</div><br />
		<div class="sub-field">
				<label for="download_place_file">&nbsp;<?php _e( 'Place of file', 'xili-dictionary' ); ?>:&nbsp;
				<select name="download_place_file" id="download_place_file">
					<option value="parent" ><?php _e('Parent theme', 'xili-dictionary'); ?></option>
					<?php if ( is_child_theme() ) { ?>
					<option value="child" ><?php _e('Child theme', 'xili-dictionary'); ?></option>
					<?php } ?>
				</select>
				</label>
				<p class="description"><?php _e( 'Place of file: .mo or .po', 'xili-dictionary' ); ?></p>
		</div><br />
		<?php $extend = 'zip' ?>
		<div class="sub-field">
...
*/
	...
	*/
	...
	echo 'var curthemename = "'.$this->get_theme_name(false).'";';
	if ( is_child_theme() )
	...
	echo 'var plugindatas = {';
		...
		foreach ($list_plugins as $plugin_path => $one_plugin) {
			...
			echo '"'. $plugin_path. '" : "' . $plugin_text_domain . '", ' ;
		}
	echo '};';
...
jQuery(document).ready( function() {
	...
	if ( place == 'local' ) {
		...
		if ( la == pluginpotfile ) {
			jQuery("#xd-looping-start").val( t+' : ' + plugindatas[plugin] + '.pot' );
		} else {
			jQuery("#xd-looping-start").val( t+' : ' + plugindatas[plugin] + '-'+la+'.'+x );
		}

		jQuery("#_origin_theme").val( plugin );
		jQuery(".title_origin_theme").html( plugindatas[plugin] );
		//
	} else {
...
?>
	...
	<div id="plugins-option" class="sub-field" style="display:none;" >
		...
		$list = array_keys( $this->xili_settings['domains'] ) ;
		...
		foreach ($list_plugins as $key_plugin => $one_plugin) {
			$text_domain = ("" != $one_plugin['TextDomain']) ? $one_plugin['TextDomain'] : __('n. a.', 'xili-dictionary');
			$active = (is_plugin_active($key_plugin)) ? ' (' . __('active','xili-dictionary') . ')' : "";
			echo '<option value="'. $key_plugin.'"> '. $one_plugin['Name'] .' {'.$text_domain.'}' . $active . '</option>';
		}
...
<div class="sub-field">
	<label for="_origin_theme"><?php _e( 'Name of current theme', 'xili-dictionary' ); ?>&nbsp;: <span class="title_origin_theme"><?php echo $this->get_theme_name(false); ?></span>
	<input id="_origin_theme" name="_origin_theme" type="hidden" id="_origin_theme" value="<?php echo $this->get_theme_name(false); ?>" /></label>
...
?>
	...
	function importing_process_callback () {
		...
		if ( $local_file ==  'plugin' ) {
			$plugin_path = ( ! empty( $_POST['_xd_plugin']) ) ? $_POST['_xd_plugin'] : 'undefined';
			$pomofile =  $this->path_plugin_file( $plugin_path, $lang, $type ) ;
		}
	...
	function path_plugin_file( $plugin_path, $lang, $type = 'po' ) {
	...
	function detect_plugin_textdomain_sub_folder ( $plugin_path ) {
		//
		$this->plugin_text_domain = "nopotfile";
		$full_path = str_replace ( '//' ,'/' , WP_PLUGIN_DIR . '/' . str_replace (basename($plugin_path),'', $plugin_path) );
	 	$this->find_files($full_path, '/^.*\.(pot)$/', array(&$this,'plugin_textdomain_folder')) ;
	 	return $this->plugin_text_domain ;
	}

	function plugin_textdomain_folder( $path, $filename ) {
				$this->plugin_text_domain = str_replace ('.pot','',$filename) ;
	}
